...
class Ball{
    ...
    update = () => {
        if(this.x + this.ballRadius + this.dx > this.canvas.width || (this.x + this.dx) - this.ballRadius < 0) this.dx = -this.dx

        if((this.y + this.dy) - this.ballRadius <= 0) this.dy = -this.dy
        
        this.x += this.dx
        this.y += this.dy
    }

    draw = () => this.renderer.drawCircle(this.x, this.y,this.ballRadius, this.color)
    ...
    checkBottomWallCollision = () => this.y + this.ballRadius > this.canvas.height ? true : false
    
    brickCollision = () => this.dy = -this.dy
